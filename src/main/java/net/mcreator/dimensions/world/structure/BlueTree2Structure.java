...
public class BlueTree2Structure extends DimensionsElements.ModElement {
	public BlueTree2Structure(DimensionsElements instance) {
		super(instance, 15);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 500000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("dimensions", "bluetree2"));
						if (template == null)
							return false;
