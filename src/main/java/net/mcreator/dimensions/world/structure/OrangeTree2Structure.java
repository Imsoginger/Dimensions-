...
public class OrangeTree2Structure extends DimensionsElements.ModElement {
	public OrangeTree2Structure(DimensionsElements instance) {
		super(instance, 17);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 500000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("dimensions", "orangetree2"));
						if (template == null)
							return false;
