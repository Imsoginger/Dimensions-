...
public class RedTree2Structure extends DimensionsElements.ModElement {
	public RedTree2Structure(DimensionsElements instance) {
		super(instance, 19);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 500000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("dimensions", "redtree2"));
						if (template == null)
							return false;
