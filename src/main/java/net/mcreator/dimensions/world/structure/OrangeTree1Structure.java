...
public class OrangeTree1Structure extends DimensionsElements.ModElement {
	public OrangeTree1Structure(DimensionsElements instance) {
		super(instance, 16);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 500000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("dimensions", "orangetree1"));
						if (template == null)
							return false;
